...
export class WorkflowTicketPerformanceReportClient extends Component {
    setup() {
    ...
    async load() {
        const res = await this.orm.call(
            "ss.report.workflow_ticket_performance",
            "get_report",
            [[]],
            {
    ...
    openTicket(ev) {
        const ticketId = Number(ev.currentTarget.dataset.ticketId);
        if (!ticketId) {
            return;
        }
    ...
    async print_pdf() {
        ...
        download({
            url: `/ss_workflow_reports/ticket_performance/pdf?${params.toString()}`,
            data: {},
        });
    }
    ...
    print_xlsx() {
        ...
        window.open(`/ss_workflow_reports/ticket_performance/xlsx?${params.toString()}`, "_blank");
    }
}

WorkflowTicketPerformanceReportClient.template = "ss_workflow.ticket_performance_report";

registry.category("actions").add(
    "ss_workflow.ticket_performance_report",
    WorkflowTicketPerformanceReportClient
);
