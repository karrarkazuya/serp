@extends('layouts.app')
@section('title', __('inventory.locations'))
...
<div class="flex flex-col h-full bg-white">
    <div class="flex flex-wrap items-center gap-2 sm:gap-3 px-3 sm:px-4 py-2 border-b border-gray-200 bg-white shrink-0">
        ...
        <a href="{{ route('inventory.config.locations.create') }}" class="px-3 sm:px-4 py-2 bg-[#714B67] hover:bg-[#5c3d55] text-white text-sm font-semibold rounded shadow-sm shrink-0">{{ __('inventory.new') }}</a>
        @endcan
        <span class="text-xl font-semibold text-gray-700 shrink-0">{{ __('inventory.locations') }}</span>
        ...
        <div class="ms-auto flex items-center gap-2 shrink-0">
            ...
            @elseif(isset($locations) && $locations->total() > 0)
    ...
    @if(isset($groups))
    <x-list :grouped="true" empty-text="{{ __('inventory.no_locations') }}">
        <x-slot:columns>
            <x-sortable-th column="complete_name" :label="__('inventory.location')" class="px-4 py-2" :default="true" />
        ...
        <tbody x-data="{ open: {{ $loop->first ? 'true' : 'false' }} }" class="divide-y divide-gray-100">
            ...
            <tr x-show="open" class="hover:bg-purple-50/30 cursor-pointer" onclick="window.location='{{ route('inventory.config.locations.show', $location) }}'">
                <td class="px-4 py-2 font-medium text-gray-900">
                    {{ $location->complete_name }}
                    @if(!$location->active) <span class="ms-1 text-[10px] text-amber-600 font-semibold uppercase">{{ __('inventory.status_archived') }}</span> @endif
                </td>
                <td class="px-3 py-2 text-sm text-gray-600">{{ $location->usage_label }}</td>
                <td class="px-3 py-2 text-sm text-gray-600">{{ $location->company?->name ?? __('inventory.all_companies') }}</td>
            </tr>
            @endforeach
        </tbody>
        @empty
        <tbody>
            <tr><td colspan="99" class="px-4 py-20 text-center text-sm text-gray-400">{{ __('inventory.no_locations') }}</td></tr>
        </tbody>
        @endforelse
    </x-list>

    @else
    <x-list :paginator="$locations" empty-text="{{ __('inventory.no_locations') }}">
        <x-slot:columns>
            <x-sortable-th column="complete_name" :label="__('inventory.location')" class="px-4 py-2" :default="true" />
            <th class="px-3 py-2 text-xs font-semibold text-gray-500 uppercase tracking-wide text-start">{{ __('inventory.col_type') }}</th>
            <th class="px-3 py-2 text-xs font-semibold text-gray-500 uppercase tracking-wide text-start">{{ __('inventory.col_company') }}</th>
