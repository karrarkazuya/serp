...
<div class="flex flex-col h-full bg-gray-50">
    ...
    <x-toolbar
        ...
        <x-slot:actions>
            <div class="flex items-center gap-2">
                @can('update', $location)
                <a href="{{ route('inventory.config.locations.edit', $location) }}" class="px-3 py-1.5 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded hover:bg-gray-50">Edit</a>
                @if($location->active)
    ...
    <div class="flex-1 overflow-y-auto">
        <div class="bg-white mx-4 mt-4 rounded-xl border border-gray-200 shadow-sm p-6">
            @if(!$location->active)
            <div class="mb-4"><div class="text-sm text-amber-700 bg-amber-50 border border-amber-200 rounded-lg px-3 py-2">This location is archived.</div></div>
            @endif

            <h1 class="text-2xl font-bold text-gray-900 mb-6">{{ $location->complete_name }}</h1>
            @foreach([
                ['Parent', $location->parent?->complete_name],
                ['Type', $location->usage_label],
                ['Company', $location->company?->name ?? 'All Companies'],
                ['Scrap Location', $location->scrap_location ? 'Yes' : 'No'],
                ['Return Location', $location->return_location ? 'Yes' : 'No'],
            ] as [$label, $value])
            ...
            <div class="mt-4">
                <h3 class="text-sm font-semibold text-gray-700 mb-2">Sub-locations</h3>
