...
<div class="flex flex-col h-full bg-gray-50">
    <form method="POST" action="{{ route('inventory.config.warehouses.store') }}" class="flex flex-col h-full">
        @csrf
        <x-toolbar>
            <x-slot:breadcrumb>
            ...
            <x-slot:actions>
                <div class="flex items-center gap-2">
                    <a href="{{ route('inventory.config.warehouses.index') }}" class="px-3 py-1.5 text-sm text-gray-600 border border-gray-300 rounded hover:bg-gray-50">Discard</a>
        ...
        <div class="flex-1 overflow-y-auto">
            <div class="bg-white mx-4 mt-4 mb-4 rounded-xl border border-gray-200 shadow-sm p-6">
                ...
                <div class="mb-5">
                    <input type="text" name="name" value="{{ old('name') }}" required placeholder="Warehouse Name"
                ...
                <p class="mt-4 text-xs text-gray-400">Saving this warehouse will automatically create its default locations (View, Stock, Input, Output, Packing) and operation types (Receipts, Delivery Orders, Internal Transfers).</p>
            </div>
        </div>
    </form>
</div>
