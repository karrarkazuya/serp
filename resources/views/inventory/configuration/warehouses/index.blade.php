@extends('layouts.app')
@section('title', __('inventory.warehouses'))
...
<div class="flex flex-col h-full bg-white">
    <div class="flex flex-wrap items-center gap-2 sm:gap-3 px-3 sm:px-4 py-2 border-b border-gray-200 bg-white shrink-0">
        ...
        <a href="{{ route('inventory.config.warehouses.create') }}" class="px-3 sm:px-4 py-2 bg-[#714B67] hover:bg-[#5c3d55] text-white text-sm font-semibold rounded shadow-sm shrink-0">{{ __('inventory.new') }}</a>
        @endcan
        <span class="text-xl font-semibold text-gray-700 shrink-0">{{ __('inventory.warehouses') }}</span>
        ...
        <div class="ms-auto flex items-center gap-2 shrink-0">
            ...
            @elseif(isset($warehouses) && $warehouses->total() > 0)
            <span class="text-sm font-semibold text-gray-600">{{ $warehouses->firstItem() }}-{{ $warehouses->lastItem() }} / {{ $warehouses->total() }}</span>
            @endif
        </div>
    </div>

    @if(isset($groups))
    <x-list :grouped="true" empty-text="{{ __('inventory.no_warehouses') }}">
        <x-slot:columns>
            <x-sortable-th column="name" :label="__('inventory.col_name')" class="px-4 py-2" :default="true" />
            <th class="px-3 py-2 text-xs font-semibold text-gray-500 uppercase tracking-wide text-start">{{ __('inventory.col_short_name') }}</th>
        ...
        <tbody x-data="{ open: {{ $loop->first ? 'true' : 'false' }} }" class="divide-y divide-gray-100">
            ...
            @foreach($group['items'] as $warehouse)
            <tr x-show="open" class="hover:bg-purple-50/30 cursor-pointer" onclick="window.location='{{ route('inventory.config.warehouses.show', $warehouse) }}'">
        ...
        @empty
        <tbody>
            <tr><td colspan="99" class="px-4 py-20 text-center text-sm text-gray-400">{{ __('inventory.no_warehouses') }}</td></tr>
        </tbody>
        @endforelse
    </x-list>

    @else
    <x-list :paginator="$warehouses" empty-text="{{ __('inventory.no_warehouses') }}">
        <x-slot:columns>
            <x-sortable-th column="name" :label="__('inventory.col_name')" class="px-4 py-2" :default="true" />
            <th class="px-3 py-2 text-xs font-semibold text-gray-500 uppercase tracking-wide text-start">{{ __('inventory.col_short_name') }}</th>
            <th class="px-3 py-2 text-xs font-semibold text-gray-500 uppercase tracking-wide text-start">{{ __('inventory.col_company') }}</th>
