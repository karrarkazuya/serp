@extends('layouts.app')
@section('title', 'Edit Reorder Rule')
...
<div class="flex flex-col h-full bg-gray-50">
    <form method="POST" action="{{ route('inventory.replenishment.update', $reorderRule) }}" class="flex flex-col h-full">
        ...
        <x-toolbar>
            <x-slot:breadcrumb>
                <a href="{{ route('inventory.replenishment.index') }}" class="text-xs text-purple-600 hover:text-purple-700">Replenishment</a>
                <span class="text-sm font-semibold text-gray-800">Edit Reorder Rule</span>
            </x-slot:breadcrumb>
            <x-slot:actions>
                <div class="flex items-center gap-2">
                    <a href="{{ route('inventory.replenishment.index') }}" class="px-3 py-1.5 text-sm text-gray-600 border border-gray-300 rounded hover:bg-gray-50">Discard</a>
            ...
            <div class="bg-white mx-4 mt-4 mb-4 rounded-xl border border-gray-200 shadow-sm p-6">
                ...
                <div class="grid grid-cols-2 gap-x-8">
                    <div>
                        ...
                        <div class="flex items-center gap-4 py-2 border-b border-gray-100">
                            <label class="w-40 shrink-0 text-sm text-gray-500">Location</label>
                            <span class="flex-1 text-sm text-gray-800">{{ $reorderRule->location?->complete_name ?? '-' }}</span>
                        </div>
                        <div class="flex items-center gap-4 py-2 border-b border-gray-100">
                            <label class="w-40 shrink-0 text-sm text-gray-500">Route</label>
                    ...
                    <div>
                        <div class="flex items-center gap-4 py-2 border-b border-gray-100">
                            <label class="w-40 shrink-0 text-sm text-gray-500">Min Quantity</label>
                        ...
                        <div class="flex items-center gap-4 py-2 border-b border-gray-100">
                            <label class="w-40 shrink-0 text-sm text-gray-500">Max Quantity</label>
                        ...
                        <div class="flex items-center gap-4 py-2 border-b border-gray-100">
                            <label class="w-40 shrink-0 text-sm text-gray-500">Qty Multiple</label>
                        ...
                        <div class="flex items-center gap-4 py-2 border-b border-gray-100">
                            <label class="w-40 shrink-0 text-sm text-gray-500">Lead Time (days)</label>
