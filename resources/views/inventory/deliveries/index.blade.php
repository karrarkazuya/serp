@extends('layouts.app')
@section('title', __('inventory.deliveries'))
...
<div class="flex flex-col h-full bg-white">
    <div class="flex flex-wrap items-center gap-2 sm:gap-3 px-3 sm:px-4 py-2 border-b border-gray-200 bg-white shrink-0">
        ...
        <a href="{{ route('inventory.deliveries.create') }}" class="px-3 sm:px-4 py-2 bg-[#714B67] hover:bg-[#5c3d55] text-white text-sm font-semibold rounded shadow-sm shrink-0">{{ __('inventory.new') }}</a>
        @endcan

        <span class="text-xl font-semibold text-gray-700 shrink-0">{{ __('inventory.deliveries') }}</span>

        <x-search :model="\App\Models\Inventory\Picking::class" :action="route('inventory.deliveries.index')" />

        <div class="ms-auto flex items-center gap-2 shrink-0">
            @if(isset($groups))
                <span class="text-sm font-semibold text-gray-600">{{ $groups->sum('count') }} {{ __('inventory.deliveries') }}</span>
    ...
    <div class="flex items-center gap-1 px-4 py-2 border-b border-gray-200 bg-white text-sm">
        ...
        <a href="{{ route('inventory.deliveries.index', array_merge(request()->except('state', 'page'), ['state' => $state])) }}"
    ...
    @if(isset($groups))
    <x-list :grouped="true" empty-text="{{ __('inventory.no_deliveries') }}">
        <x-slot:columns>
            <x-sortable-th column="name" :label="__('inventory.col_reference')" class="px-4 py-2" :default="true" />
            <th class="px-3 py-2 text-xs font-semibold text-gray-500 uppercase tracking-wide text-start">{{ __('inventory.col_customer') }}</th>
        ...
        @empty
        <tbody>
            <tr><td colspan="99" class="px-4 py-20 text-center text-sm text-gray-400">{{ __('inventory.no_deliveries') }}</td></tr>
        </tbody>
        @endforelse
    </x-list>

    @else
    <x-list :paginator="$pickings" empty-text="{{ __('inventory.no_deliveries') }}">
        <x-slot:columns>
            <x-sortable-th column="name" :label="__('inventory.col_reference')" class="px-4 py-2" :default="true" />
            <th class="px-3 py-2 text-xs font-semibold text-gray-500 uppercase tracking-wide text-start">{{ __('inventory.col_customer') }}</th>
            <th class="px-3 py-2 text-xs font-semibold text-gray-500 uppercase tracking-wide text-start">{{ __('inventory.col_from') }}</th>
