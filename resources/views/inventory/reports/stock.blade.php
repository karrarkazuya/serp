...
<div class="flex flex-col h-full bg-white">
    <div class="flex flex-wrap items-center gap-2 sm:gap-3 px-3 sm:px-4 py-2 border-b border-gray-200 bg-white shrink-0">
        <span class="text-xl font-semibold text-gray-700 shrink-0">Stock Report</span>
        ...
        <form method="GET" action="{{ route('inventory.reports.stock') }}" class="flex items-center gap-2 ms-4">
            ...
            <button type="submit" class="px-3 py-1.5 text-sm bg-purple-600 text-white rounded hover:bg-purple-700">Apply</button>
            <a href="{{ route('inventory.reports.stock') }}" class="px-3 py-1.5 text-sm text-gray-600 border border-gray-300 rounded hover:bg-gray-50">Clear</a>
        </form>
    ...
    <x-list :paginator="$quants" empty-text="No stock found.">
        <x-slot:columns>
            ...
            <th class="px-3 py-2 text-xs font-semibold text-gray-500 uppercase tracking-wide text-left">Lot / Serial</th>
            <th class="px-3 py-2 text-xs font-semibold text-gray-500 uppercase tracking-wide text-right">On Hand</th>
            <th class="px-3 py-2 text-xs font-semibold text-gray-500 uppercase tracking-wide text-right">Reserved</th>
            <th class="px-3 py-2 text-xs font-semibold text-gray-500 uppercase tracking-wide text-right">Available</th>
        </x-slot:columns>
        ...
        <tr class="hover:bg-purple-50/30">
            ...
            <td class="px-3 py-2 text-sm text-gray-800 text-right font-medium">{{ number_format($quant->qty, 2) }}</td>
            <td class="px-3 py-2 text-sm text-amber-600 text-right">{{ number_format($quant->reserved_qty, 2) }}</td>
            <td class="px-3 py-2 text-sm text-right font-semibold {{ $quant->getAvailableQty() < 0 ? 'text-red-600' : 'text-green-700' }}">
