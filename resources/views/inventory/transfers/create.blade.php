@extends('layouts.app')
@section('title', __('inventory.new_transfer'))
...
<div class="flex flex-col h-full bg-gray-50">
    <form method="POST" action="{{ route('inventory.transfers.store') }}" class="flex flex-col h-full">
        ...
        <x-toolbar>
            <x-slot:breadcrumb>
                <a href="{{ route('inventory.transfers.index') }}" class="text-xs text-purple-600 hover:text-purple-700">{{ __('inventory.transfers') }}</a>
                <span class="text-sm font-semibold text-gray-800">{{ __('inventory.new') }}</span>
            </x-slot:breadcrumb>
            <x-slot:actions>
                <div class="flex items-center gap-2">
                    <a href="{{ route('inventory.transfers.index') }}" class="px-3 py-1.5 text-sm text-gray-600 border border-gray-300 rounded hover:bg-gray-50">{{ __('inventory.discard') }}</a>
            ...
            <div class="bg-white mx-4 mt-4 mb-4 rounded-xl border border-gray-200 shadow-sm p-6">
                ...
                <div class="grid grid-cols-2 gap-x-8">
                    <div>
                        ...
                        <div class="flex items-center gap-4 py-2 border-b border-gray-100">
                            <label class="w-40 shrink-0 text-sm text-gray-500">{{ __('inventory.destination_location') }}</label>
                ...
                <div class="mt-6">
                    ...
                    <table class="w-full text-sm mb-3">
                        ...
                        <tbody x-data="{ nextIdx: {{ count(old('moves', [])) }}, newMoveRowUrl: @js(route('inventory.transfers.new-move-row')) }">
                            {{-- Repopulate rows after validation error --}}
