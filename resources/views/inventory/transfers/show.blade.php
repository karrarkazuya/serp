    ...
    <x-toolbar
        ...
        <x-slot:actions>
            <div class="flex items-center gap-2">
                @if($picking->canEdit())
                    <a href="{{ route('inventory.transfers.edit', $picking) }}" class="px-3 py-1.5 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded hover:bg-gray-50">Edit</a>
                @endif
                @can('update', $picking)
                    ...
                    <form method="POST" action="{{ route('inventory.transfers.check-availability', $picking) }}">
                        @csrf
                        <button class="px-3 py-1.5 text-sm font-semibold text-white bg-blue-600 hover:bg-blue-700 rounded">Check Availability</button>
                    </form>
                    ...
                    <form method="POST" action="{{ route('inventory.transfers.cancel', $picking) }}" x-data="{ confirming: false }">
                        @csrf
                        <button type="button" x-show="!confirming" @click="confirming = true" class="px-3 py-1.5 text-sm text-red-700 border border-red-200 rounded hover:bg-red-50">Cancel</button>
                        <div x-show="confirming" style="display:none" class="flex items-center gap-1.5">
                            <span class="text-xs text-red-600">Cancel transfer?</span>
                            <button type="submit" class="px-2 py-1 text-xs bg-red-600 text-white rounded">Yes</button>
                            <button type="button" @click="confirming = false" class="px-2 py-1 text-xs text-gray-500 border rounded">No</button>
                        </div>
                    </form>
                    @endif
                    ...
                    <form method="POST" action="{{ route('inventory.transfers.return', $picking) }}" x-data="{ confirming: false }">
                        @csrf
                        <button type="button" x-show="!confirming" @click="confirming = true" class="px-3 py-1.5 text-sm text-gray-700 border border-gray-300 rounded hover:bg-gray-50">Return</button>
                        <div x-show="confirming" style="display:none" class="flex items-center gap-1.5">
                            <span class="text-xs text-gray-600">Create return?</span>
                            <button type="submit" class="px-2 py-1 text-xs bg-gray-700 text-white rounded">Yes</button>
                            <button type="button" @click="confirming = false" class="px-2 py-1 text-xs text-gray-500 border rounded">No</button>
                        </div>
                    </form>
                    @endif
    ...
    <div class="flex-1 overflow-y-auto">
        <div class="bg-white mx-4 mt-4 rounded-xl border border-gray-200 shadow-sm">
            ...
            <div class="px-6 py-3 border-b border-gray-100 flex items-center gap-6">
                @foreach(['Draft' => 'draft', 'Confirmed' => 'confirmed', 'Ready' => 'assigned', 'Done' => 'done'] as $label => $state)
                <div class="flex items-center gap-2">
            ...
            <div class="p-6">
                <div class="grid grid-cols-2 gap-x-8 gap-y-0">
                    <div>
                        @foreach([
                            ['Operation Type', $picking->operationType?->name],
                            ['From',           $picking->srcLocation?->complete_name],
                            ['To',             $picking->destLocation?->complete_name],
                            ['Scheduled Date', $picking->scheduled_date?->format('M d, Y H:i')],
                        ] as [$label, $value])
                    ...
                    <div>
                        @foreach([
                            ['Source Document', $picking->origin],
                            ['Company',         $picking->company?->name],
                        ] as [$label, $value])
                        <div class="flex items-center gap-4 py-2 border-b border-gray-100">
                            <span class="w-40 shrink-0 text-sm text-gray-500">{{ $label }}</span>
                            <span class="flex-1 text-sm text-gray-800">{{ $value ?: '-' }}</span>
                        ...
                        <div class="flex items-start gap-4 py-2 border-b border-gray-100">
                            <span class="w-40 shrink-0 text-sm text-gray-500">Note</span>
            ...
            <div class="border-t border-gray-200">
                <div class="px-6 py-3 text-sm font-semibold text-gray-700">Operations</div>
                <div class="overflow-x-auto">
                    <table class="w-full text-sm">
                        <thead class="border-b border-gray-200">
                            <tr>
                                <th class="px-6 py-2 text-left text-xs font-semibold text-gray-500">Product</th>
                                <th class="px-3 py-2 text-left text-xs font-semibold text-gray-500">Lot / Serial</th>
                                <th class="px-3 py-2 text-right text-xs font-semibold text-gray-500">Demand</th>
                                @if($picking->isConfirmed() || $picking->isAssigned() || $picking->isDone())
                                <th class="px-3 py-2 text-right text-xs font-semibold text-gray-500">Done</th>
                                @endif
                                <th class="px-3 py-2 text-left text-xs font-semibold text-gray-500">UoM</th>
                            </tr>
                        </thead>
                        <tbody>
                            ...
                            <tr class="border-b border-gray-100">
                                ...
                                <td class="px-3 py-2 text-right text-gray-800">{{ number_format($move->product_qty, 2) }}</td>
                                @if($picking->isConfirmed() || $picking->isAssigned() || $picking->isDone())
                            ...
                            @empty
                            <tr><td colspan="5" class="px-6 py-8 text-center text-gray-400 text-sm">No operations yet.</td></tr>
                            @endforelse
                        </tbody>
                    </table>
                </div>
                ...
                <div class="px-6 py-3 border-t border-gray-100 flex items-center gap-4">
                    <button form="validate-form" type="submit" class="px-4 py-2 text-sm font-semibold text-white bg-green-600 hover:bg-green-700 rounded">Validate Transfer</button>
