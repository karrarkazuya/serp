...
<div class="flex flex-col h-full bg-gray-50">
    ...
    <x-toolbar
        ...
        <x-slot:breadcrumb>
            <a href="{{ route('inventory.products.index') }}" class="text-xs text-purple-600 hover:text-purple-700">Products</a>
            <span class="text-sm font-semibold text-gray-800">{{ $product->name }}</span>
        </x-slot:breadcrumb>
        <x-slot:actions>
            <div class="flex items-center gap-2">
                @can('update', $product)
                <a href="{{ route('inventory.products.edit', $product) }}" class="px-3 py-1.5 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded hover:bg-gray-50">Edit</a>
                @if($product->active)
    ...
    <div class="flex-1 overflow-y-auto">
        <div class="bg-white mx-4 mt-4 rounded-xl border border-gray-200 shadow-sm">
            @if(!$product->active)
            <div class="px-6 pt-4"><div class="text-sm text-amber-700 bg-amber-50 border border-amber-200 rounded-lg px-3 py-2">This product is archived.</div></div>
            @endif
            ...
            <div class="p-6">
                <div class="flex gap-8">
                    <div class="flex-1">
                        ...
                        @foreach([
                            ['Product Type', $product->product_type_label],
                            ['Category', $product->category?->name],
                            ['Unit of Measure', $product->uom?->name],
                            ['Purchase UoM', $product->uomPo?->name],
                            ['Barcode', $product->barcode],
                            ['Tracking', ucfirst($product->tracking ?? 'None')],
                            ['Sales Price', number_format($product->sale_price ?? 0, 2)],
                    ...
                    <div class="shrink-0 text-right">
                        ...
                        <div class="mt-4 text-center">
                            <a href="{{ route('inventory.reports.stock', ['product_id' => $product->id]) }}" class="text-xs text-purple-600 hover:underline">
                                {{ number_format($quants->sum('quantity'), 2) }} on hand
                            </a>
                        </div>
                    </div>
                </div>
                ...
                <div class="mt-4 flex items-center gap-4 py-2 border-b border-gray-100">
                    <span class="w-40 shrink-0 text-sm text-gray-500">Routes</span>
                ...
                <div class="mt-6">
                    <h3 class="text-sm font-semibold text-gray-700 mb-2">Vendors</h3>
                    <table class="w-full text-sm">
                        <thead><tr class="border-b border-gray-200">
                            <th class="py-1.5 text-left text-xs text-gray-500">Vendor</th>
                            <th class="py-1.5 text-right text-xs text-gray-500">Price</th>
                            <th class="py-1.5 text-right text-xs text-gray-500">Min Qty</th>
                            <th class="py-1.5 text-right text-xs text-gray-500">Lead Time</th>
                        </tr></thead>
                        <tbody>
                            @foreach($product->suppliers as $supplier)
                            <tr class="border-b border-gray-100">
                                <td class="py-1.5 text-gray-800">{{ $supplier->partner?->name ?? $supplier->partner_name }}</td>
                                <td class="py-1.5 text-right text-gray-800">{{ number_format($supplier->price, 2) }}</td>
                                <td class="py-1.5 text-right text-gray-600">{{ $supplier->min_qty }}</td>
                                <td class="py-1.5 text-right text-gray-600">{{ $supplier->delay }} days</td>
                            </tr>
                            @endforeach
                        </tbody>
                    </table>
