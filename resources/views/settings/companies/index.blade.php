...
<div class="flex flex-col h-full">
    ...
    <div class="bg-white border-b border-gray-200 px-4 py-2 flex items-center gap-3 flex-wrap shrink-0">
        ...
        <div class="ms-auto flex items-center gap-3 text-sm text-gray-500 shrink-0">
            ...
            @elseif(isset($companies) && $companies->total() > 0)
            ...
            <div class="flex items-center gap-1">
                @if(isset($companies) && $companies->onFirstPage())
                ...
                @endif
                @if(isset($companies) && $companies->hasMorePages())
    ...
    @if(isset($groups))
    <x-list :grouped="true" :empty-text="__('settings.no_companies')">
        <x-slot:columns>
        ...
        <tbody x-data="{ open: {{ $loop->first ? 'true' : 'false' }} }" class="divide-y divide-gray-100">
            ...
            @foreach($group['items'] as $company)
            <tr x-show="open" class="hover:bg-[#714B67]/5 transition-colors cursor-pointer" onclick="window.location='{{ route('settings.companies.show', $company) }}'">
                <td class="px-6 py-3">
        ...
        @empty
        <tbody>
            <tr><td colspan="99" class="px-4 py-20 text-center text-sm text-gray-400">{{ __('settings.no_companies') }}</td></tr>
        </tbody>
        @endforelse
    </x-list>
