...
<div class="bg-white rounded-xl border border-gray-200 p-6">
    <h2 class="text-sm font-semibold text-gray-700 mb-4">User Information</h2>
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div>
            <label class="block text-xs font-medium text-gray-600 mb-1">Job Position</label>
...
<div class="bg-white rounded-xl border border-gray-200 p-6">
    <h2 class="text-sm font-semibold text-gray-700 mb-1">Password</h2>
    @if($user)
        <p class="text-xs text-gray-500 mb-4">Leave blank to keep the current password.</p>
    @else
        <p class="text-xs text-gray-500 mb-4">Set a secure password for this user.</p>
    @endif
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
            <label class="block text-xs font-medium text-gray-600 mb-1">Password {{ !$user ? '<span class="text-red-500">*</span>' : '' }}</label>
...
<div class="bg-white rounded-xl border border-gray-200 p-6">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-2">
        ...
        @empty
        <p class="text-sm text-gray-400 col-span-2">No roles available. <a href="{{ route('settings.roles.create') }}" class="text-purple-600 hover:underline">Create one</a>.</p>
        @endforelse
    </div>
</div>
