    ...
    <div class="flex min-w-0 {{ $isAppLauncher ? 'h-screen' : 'h-[calc(100vh-52px)]' }}">
        ...
        <main class="min-w-0 flex-1 overflow-y-auto">
            ...
            <div x-data="{ show: false, message: '', type: 'success', timer: null }"
                 ...
                 x-show="show"
                 x-transition
                 class="fixed {{ $isAppLauncher ? 'top-4' : 'top-14' }} right-4 z-50 flex items-center gap-2 text-white px-4 py-3 rounded-lg shadow-lg text-sm font-medium transition-all"
            ...
            @if(session('success'))
                <div x-data="{ show: true }" x-show="show" x-init="setTimeout(() => show = false, 4000)"
                     class="fixed {{ $isAppLauncher ? 'top-4' : 'top-14' }} right-4 z-40 flex items-center gap-2 bg-green-600 text-white px-4 py-3 rounded-lg shadow-lg text-sm font-medium transition-all">
            ...
            @if(session('error'))
                <div x-data="{ show: true }" x-show="show" x-init="setTimeout(() => show = false, 5000)"
                     class="fixed {{ $isAppLauncher ? 'top-4' : 'top-14' }} right-4 z-40 flex items-center gap-2 bg-red-600 text-white px-4 py-3 rounded-lg shadow-lg text-sm font-medium">
