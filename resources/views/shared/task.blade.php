...
<head>
    ...
    <style>
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .banner-pending   { background: linear-gradient(135deg, #0868b8, #1a8fd1); }
        .banner-completed { background: linear-gradient(135deg, #0a8f52, #12b863); }
        .banner-rejected  { background: linear-gradient(135deg, #b91c1c, #dc2626); }
        .banner-skipped   { background: linear-gradient(135deg, #3a3f47, #545b64); }
        .banner-draft     { background: linear-gradient(135deg, #4b5563, #6b7280); }

        .banner h1 { font-size: 20px; font-weight: 700; color: #fff; line-height: 1.35; }
        .banner-sub { font-size: 13px; color: rgba(255,255,255,0.65); margin-top: 4px; }
        ...
        .message-section { padding: 24px 28px 28px; }
        .message-label {
            font-size: 10px;
            font-weight: 700;
            letter-spacing: 2px;
        ...
        @media (max-width: 540px) {
            ...
            .fields, .message-section { padding: 18px; }
        ...
        <div class="card">
            <div class="banner banner-{{ $task->state }}">
                <div>
                    <h1>{{ $task->name }}</h1>
                    @if($task->procedure)
                    <div class="banner-sub">{{ $task->procedure->name }}</div>
                    @endif
                </div>
                <span class="pill">{{ $task->stateLabel() }}</span>
            </div>
            ...
            <div class="fields">
                ...
                <div class="field-row">
                    ...
                    <span class="field-value">{{ $input->isFilled() ? $input->getResultValue() : '—' }}</span>
                </div>
                @endforeach
            </div>
            @endif
