...
<div class="flex flex-col h-full bg-gray-50">
    ...
    <x-toolbar
        ...
        <x-slot:breadcrumb>
            ...
            <a href="{{ route('accounting.payment-terms.index') }}" class="text-xs text-purple-600 hover:text-purple-700">{{ __('accounting.payment_terms') }}</a>
            <span class="text-sm font-semibold text-gray-800">{{ $paymentTerm->name }}</span>
        </x-slot:breadcrumb>
        <x-slot:actions>
            <div class="flex items-center gap-2">
                @can('update', $paymentTerm)
                <a href="{{ route('accounting.payment-terms.edit', $paymentTerm) }}" class="px-3 py-1.5 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded hover:bg-gray-50">{{ __('accounting.btn_edit') }}</a>
        ...
        <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-6 max-w-2xl">
            <div class="flex items-center gap-3 mb-6">
                ...
                <span class="inline-block px-2 py-0.5 rounded text-[11px] font-medium {{ $paymentTerm->active ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500' }}">
                    {{ $paymentTerm->active ? __('accounting.status_active') : __('accounting.status_archived') }}
                </span>
            </div>
            ...
            <div class="mt-6">
                <h2 class="text-sm font-semibold text-gray-700 mb-3">{{ __('accounting.payment_terms') }}</h2>
                <table class="w-full text-sm border border-gray-200 rounded">
                    <thead class="bg-gray-50">
                        <tr>
                            <th class="px-3 py-2 text-left text-xs font-semibold text-gray-500 uppercase">{{ __('accounting.col_type') }}</th>
                            <th class="px-3 py-2 text-left text-xs font-semibold text-gray-500 uppercase">{{ __('accounting.col_rate_amount') }}</th>
                            <th class="px-3 py-2 text-left text-xs font-semibold text-gray-500 uppercase">{{ __('accounting.col_due_date') }}</th>
                        </tr>
                    </thead>
                    <tbody>
