...
<div class="flex flex-col h-full bg-gray-50">
    <x-toolbar>
        <x-slot:breadcrumb>
            ...
            <a href="{{ route($config['routes']['show'], $document) }}" class="text-xs text-purple-600 hover:text-purple-700">{{ $document->name ?: 'Draft' }}</a>
        ...
        <x-slot:actions>
            <div class="flex items-center gap-2">
                ...
                @endif
                <button form="document-form" type="submit" name="action" value="save" class="px-3 py-1.5 text-sm text-gray-700 bg-white border border-gray-300 rounded hover:bg-gray-50">Save</button>
                <a href="{{ route($config['routes']['show'], $document) }}" class="px-3 py-1.5 text-sm text-gray-600 bg-white border border-gray-300 rounded hover:bg-gray-50">Cancel</a>
            </div>
        </x-slot:actions>
    </x-toolbar>
