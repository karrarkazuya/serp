...
@php
    ...
    $procQuickFilters = [
        ['label' => 'In Progress', 'params' => ['state' => 'pending'],   'url' => route('workflow.procedures.index', array_merge(request()->except('page'), ['state' => 'pending']))],
        ['label' => 'Completed',   'params' => ['state' => 'completed'], 'url' => route('workflow.procedures.index', array_merge(request()->except('page'), ['state' => 'completed']))],
        ['label' => 'Canceled',    'params' => ['state' => 'closed'],    'url' => route('workflow.procedures.index', array_merge(request()->except('page'), ['state' => 'closed']))],
        ['label' => 'Active',      'params' => ['filter' => ''],         'url' => route('workflow.procedures.index', array_merge(request()->except('page', 'filter'), ['filter' => '']))],
        ['label' => 'Archived',    'params' => ['filter' => 'archived'], 'url' => route('workflow.procedures.index', array_merge(request()->except('page'), ['filter' => 'archived']))],
        ['label' => 'All',         'params' => ['filter' => 'all'],      'url' => route('workflow.procedures.index', array_merge(request()->except('page'), ['filter' => 'all']))],
    ];
    $procGroups = [
        ['label' => 'State', 'url' => route('workflow.procedures.index', array_merge(request()->except('page'), ['group_by' => 'state']))],
    ];
@endphp
...
<div class="flex min-w-0 flex-col h-full {{ $view === 'kanban' ? 'bg-gray-100' : 'bg-white' }}" x-data="{ checked: [] }">
    <div class="flex flex-wrap items-center gap-2 sm:gap-3 px-3 sm:px-4 py-2 border-b border-gray-200 bg-white shrink-0">
        ...
        <div class="flex items-center gap-1.5 min-w-0 shrink-0">
            <span class="text-xl font-semibold text-gray-700">Procedures</span>
        </div>
    ...
    <div class="flex-1 overflow-y-auto p-3 sm:p-4">
        @if($procedures->isEmpty())
            <div class="py-24 text-center text-gray-400">
                <p class="text-sm font-medium text-gray-500">No procedures found</p>
            </div>
        @else
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3">
                @foreach($procedures as $procedure)
                <a href="{{ route('workflow.procedures.show', $procedure) }}" class="group bg-white rounded-xl border border-gray-200 shadow-sm hover:shadow-md transition-all overflow-hidden">
                    <div class="p-3">
    ...
    @else
    <x-list :paginator="$procedures" empty-text="No procedures found.">
        <x-slot:columns>
            ...
            <th class="px-3 py-2 text-left text-xs font-semibold text-gray-500 uppercase tracking-wide">Created By</th>
