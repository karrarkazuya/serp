...
<div class="flex flex-col h-full bg-gray-50">
    <div class="bg-white border-b border-gray-200 px-4 py-2 flex items-center gap-3 shrink-0">
        ...
        <div class="ml-auto flex items-center gap-2">
            ...
            <a href="{{ route('workflow.config.ticket-templates.edit', $ticketTemplate) }}" class="px-3 py-1.5 text-sm border border-gray-300 rounded hover:bg-gray-50">Edit</a>
            @endcan
            @can('create', \App\Models\Workflow\Ticket::class)
            <a href="{{ route('workflow.tickets.create', ['template_id' => $ticketTemplate->id]) }}"
               class="px-3 py-1.5 text-sm bg-[#714B67] text-white rounded hover:bg-[#5c3d55]">Create Ticket</a>
            @endcan
        </div>
    </div>
    ...
    <div class="flex-1 overflow-y-auto">
        ...
        <div class="bg-white mx-4 mt-4 rounded-xl border border-gray-200 shadow-sm">
            <div class="px-6 py-5">
                ...
                <div class="flex items-center gap-4 py-2 border-b border-gray-100">
                    <label class="w-36 shrink-0 text-sm text-gray-500">Default Group</label>
        ...
        <div class="bg-white mx-4 mt-4 mb-4 rounded-xl border border-gray-200 shadow-sm overflow-hidden">
            <div class="px-5 py-3 border-b border-gray-100">
                <span class="text-sm font-semibold text-gray-800">Form Fields</span>
            </div>
            <table class="w-full text-sm">
                <thead>
                    <tr class="border-b border-gray-100 bg-gray-50/70">
                        <th class="px-5 py-2.5 text-left text-xs font-semibold text-gray-400 uppercase tracking-wide">Name</th>
                        <th class="px-4 py-2.5 text-left text-xs font-semibold text-gray-400 uppercase tracking-wide">Type</th>
                        <th class="px-4 py-2.5 text-left text-xs font-semibold text-gray-400 uppercase tracking-wide">Required</th>
                        <th class="px-4 py-2.5 text-left text-xs font-semibold text-gray-400 uppercase tracking-wide">Options</th>
                    </tr>
                </thead>
                <tbody>
                    ...
                    <tr class="border-b border-gray-50">
                        ...
                        <td class="px-4 py-2.5 text-gray-600">{{ $input->is_required ? 'Yes' : 'No' }}</td>
