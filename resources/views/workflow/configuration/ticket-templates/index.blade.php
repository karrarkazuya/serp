@extends('layouts.app')
@section('title', __('workflow.ticket_templates_title'))

@section('content')
<div class="flex flex-col h-full bg-white">
    <div class="flex flex-wrap items-center gap-3 px-4 py-2 border-b border-gray-200 shrink-0">
        @can('create', \App\Models\Workflow\TicketTemplate::class)
        <a href="{{ route('workflow.config.ticket-templates.create') }}" class="px-4 py-2 bg-[#714B67] hover:bg-[#5c3d55] text-white text-sm font-semibold rounded shadow-sm">{{ __('common.new') }}</a>
        @endcan
        <span class="text-xl font-semibold text-gray-700">{{ __('workflow.ticket_templates_title') }}</span>
        <x-search
    ...
    <x-list :paginator="$templates" :empty-text="__('workflow.no_ticket_templates')">
        ...
        <tr class="hover:bg-purple-50/30 cursor-pointer" onclick="window.location='{{ route('workflow.config.ticket-templates.show', $tpl) }}'">
            <td class="px-4 py-2 font-medium text-gray-900">{{ $tpl->name }}</td>
            <td class="px-3 py-2 text-gray-600">{{ $tpl->defaultGroup?->name ?? '—' }}</td>
            <td class="px-3 py-2 text-gray-600">{{ $tpl->resolve_max_duration ?? '—' }}</td>
            <td class="px-3 py-2">
            ...
            <td class="px-3 py-2 text-right" onclick="event.stopPropagation()">
                @can('update', $tpl)
                <a href="{{ route('workflow.config.ticket-templates.edit', $tpl) }}" class="text-xs text-purple-600 hover:text-purple-700 mr-3">{{ __('common.edit') }}</a>
                @endcan
                @can('delete', $tpl)
                <form method="POST" action="{{ route('workflow.config.ticket-templates.delete', $tpl) }}" class="inline" @submit.prevent="$dispatch('confirm-delete', { message: 'Are you sure you want to delete this?', form: $el })">
                    @csrf @method('DELETE')
