    ...
    <x-list :paginator="$templates" :empty-text="__('workflow.no_ticket_templates')">
        ...
        <tr class="hover:bg-purple-50/30 cursor-pointer" onclick="window.location='{{ route('workflow.config.ticket-templates.show', $tpl) }}'">
            ...
            <td class="px-3 py-2 text-end" onclick="event.stopPropagation()">
                @can('update', $tpl)
                <a href="{{ route('workflow.config.ticket-templates.edit', $tpl) }}" class="text-xs text-purple-600 hover:text-purple-700 me-3">{{ __('common.edit') }}</a>
                @endcan
                @can('delete', $tpl)
                <form method="POST" action="{{ route('workflow.config.ticket-templates.delete', $tpl) }}" class="inline" @submit.prevent="$dispatch('confirm-delete', { message: @js(__('common.confirm_delete')), form: $el })">
                    @csrf @method('DELETE')
