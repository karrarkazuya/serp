...
<div class="flex flex-col h-full bg-white">
    <div class="flex flex-wrap items-center gap-3 px-4 py-2 border-b border-gray-200 shrink-0">
        ...
        <a href="{{ route('workflow.config.ticket-templates.create') }}" class="px-4 py-2 bg-[#714B67] hover:bg-[#5c3d55] text-white text-sm font-semibold rounded shadow-sm">New</a>
        @endcan
        <span class="text-xl font-semibold text-gray-700">Ticket Templates</span>
        <x-search
    ...
    @endif
    <x-list :paginator="$templates" empty-text="No ticket templates yet.">
        <x-slot:columns>
        ...
        <tr class="hover:bg-purple-50/30 cursor-pointer" onclick="window.location='{{ route('workflow.config.ticket-templates.show', $tpl) }}'">
            ...
            <td class="px-3 py-2 text-right" onclick="event.stopPropagation()">
                @can('update', $tpl)
                <a href="{{ route('workflow.config.ticket-templates.edit', $tpl) }}" class="text-xs text-purple-600 hover:text-purple-700 mr-3">Edit</a>
                @endcan
                @can('delete', $tpl)
