@extends('layouts.app')
@section('title', 'New Work Location')
...
<div class="flex flex-col h-full bg-gray-50">
    <div class="bg-white border-b border-gray-200 px-4 py-2 flex items-center gap-3 shrink-0">
        <div class="flex flex-col leading-tight">
            ...
            <span class="text-sm font-semibold text-gray-800">New Work Location</span>
        </div>

        <div class="ms-auto flex items-center gap-2">
            <a href="{{ route('employees.work-locations.index') }}" class="px-3 py-1.5 text-sm text-gray-600 bg-white border border-gray-300 rounded hover:bg-gray-50">Cancel</a>
            <button form="location-form" type="submit" class="px-4 py-1.5 text-sm font-medium text-white bg-[#714B67] hover:bg-[#5c3d55] rounded shadow-sm">Save</button>
        </div>
    </div>
        ...
        <div class="bg-white mx-4 my-4 rounded-xl border border-gray-200 shadow-sm p-6">
            ...
            <form id="location-form" method="POST" action="{{ route('employees.work-locations.store') }}">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-4">
                    ...
                    <div>
                        <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wide mb-1">Latitude</label>
                        <input type="number" step="any" name="latitude" value="{{ old('latitude') }}"
                               class="w-full border-b border-gray-300 focus:border-purple-500 focus:outline-none py-1.5 text-sm bg-transparent">
                    </div>
                    <div>
                        <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wide mb-1">Longitude</label>
