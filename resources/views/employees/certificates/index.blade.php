...
<div class="flex min-w-0 flex-col h-full bg-white">
    <div class="flex flex-wrap items-center gap-2 sm:gap-3 px-3 sm:px-4 py-2 border-b border-gray-200 bg-white shrink-0">
        ...
        <x-export
            :fields="config('exportable')['employees.certificates']['fields'] ?? []"
            :export-url="route('export')"
            model-key="employees.certificates"
        />
        @endcan
    ...
    @if(isset($groups))
    <x-list :grouped="true" :empty-text="__('employees.no_certificates')">
        <x-slot:columns>
            <th class="px-4 py-2 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">{{ __('employees.certificate_type') }}</th>
        ...
        <tbody x-data="{ open: {{ $loop->first ? 'true' : 'false' }} }" class="divide-y divide-gray-100">
            ...
            <tr x-show="open" class="hover:bg-purple-50/30 cursor-pointer" onclick="window.location='{{ route('employees.certificates.show', $record) }}'">
                <td class="px-4 py-2.5">
                    <p class="text-sm font-semibold text-gray-900">{{ $record->certificate_type ?? '—' }}</p>
                    @if(!$record->active)
                        <span class="inline-block px-1.5 py-0.5 rounded text-[10px] font-semibold text-amber-700 bg-amber-50">{{ __('common.archived') }}</span>
                    @endif
                </td>
                <td class="px-3 py-2.5 text-sm text-gray-600">{{ $record->employee?->name ?? '—' }}</td>
                <td class="px-3 py-2.5 text-sm text-gray-600">
                    @if($record->financial_specialization)
        ...
        @empty
        <tbody>
            <tr><td colspan="99" class="px-4 py-20 text-center text-sm text-gray-400">{{ __('employees.no_certificates') }}</td></tr>
        </tbody>
        @endforelse
    </x-list>

    @else
    <x-list :paginator="$records" :selectable="true" :total-count="$records->total()" :empty-text="__('employees.no_certificates')" :model="\App\Models\Employees\Employee::class"
            :bulk-delete-url="route('employees.certificates.bulk-delete')">
        <x-slot:columns>
            <th class="px-4 py-2 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">{{ __('employees.certificate_type') }}</th>
            <th class="px-3 py-2 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">{{ __('employees.doc_employee') }}</th>
            <th class="px-3 py-2 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">{{ __('employees.specialization_type') }}</th>
        ...
        <tr class="hover:bg-purple-50/30 cursor-pointer" onclick="window.location='{{ route('employees.certificates.show', $record) }}'">
            ...
            <td class="px-3 py-2.5 text-sm text-gray-600">
                @if($record->employee)
                    <a href="{{ route('employees.show', $record->employee) }}" class="text-purple-600 hover:underline" @click.stop>{{ $record->employee->name }}</a>
                @else —
                @endif
            </td>
            <td class="px-3 py-2.5 text-sm text-gray-600">
