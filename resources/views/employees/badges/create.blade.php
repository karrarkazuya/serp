@extends('layouts.app')
@section('title', __('employees.new_badge'))
...
<div class="flex flex-col h-full bg-gray-50">
    <x-toolbar>
        <x-slot:breadcrumb>
            ...
            <span class="text-sm font-semibold text-gray-800">{{ __('employees.new_badge') }}</span>
        </x-slot:breadcrumb>
        <x-slot:actions>
    <div class="flex items-center gap-2">
        <a href="{{ route('employees.badges.index') }}" class="px-3 py-1.5 text-sm text-gray-600 bg-white border border-gray-300 rounded hover:bg-gray-50">{{ __('common.cancel') }}</a>
    ...
    <div class="flex-1 overflow-y-auto">
        <div class="bg-white mx-4 my-4 rounded-xl border border-gray-200 shadow-sm p-6">
            ...
            @endif
            <form id="badge-form" method="POST" action="{{ route('employees.badges.store') }}">
                @csrf
