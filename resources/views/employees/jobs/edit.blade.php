...
<div class="flex flex-col h-full bg-gray-50">
    <div class="bg-white border-b border-gray-200 px-4 py-2 flex items-center gap-3 shrink-0">
        <div class="flex flex-col leading-tight">
            ...
            <div class="flex items-center gap-1">
                <a href="{{ route('employees.jobs.show', $job) }}" class="text-sm font-semibold text-gray-800 hover:text-purple-700">{{ $job->name }}</a>
        ...
        <div class="flex items-center gap-2">
            <a href="{{ route('employees.jobs.show', $job) }}" class="px-3 py-1.5 text-sm text-gray-600 bg-white border border-gray-300 rounded hover:bg-gray-50">{{ __('common.cancel') }}</a>
    ...
    <div class="flex-1 overflow-y-auto">
        <div class="bg-white mx-4 my-4 rounded-xl border border-gray-200 shadow-sm">
            <form id="job-form" method="POST" action="{{ route('employees.jobs.update', $job) }}">
                @csrf
                @method('PUT')
                @include('employees.jobs._form', ['job' => $job])
            </form>
        </div>
    </div>
</div>
