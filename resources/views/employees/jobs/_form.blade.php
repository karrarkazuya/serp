<div class="p-6 space-y-5">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-4">
        <div>
            <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wide mb-1">Job Position Name <span class="text-red-500">*</span></label>
        ...
        <div>
            ...
            <x-relation-dropdown
                ...
                :selected-id="old('department_id', $job?->department_id)"
                :selected-label="$job?->department?->name"
        ...
        <div>
            ...
            <x-relation-dropdown
                name="company_id"
                table="companies"
                :selected-id="old('company_id', $job?->company_id)"
                :selected-label="$job?->company?->name"
                placeholder="Select company..."
            />
        </div>

        <div>
            <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wide mb-1">Expected Employees</label>
            <input type="number" name="expected_employees" value="{{ old('expected_employees', $job?->expected_employees ?? 1) }}" min="0"
                   class="w-full border-b border-gray-300 focus:border-purple-500 focus:outline-none py-1.5 text-sm bg-transparent">
        </div>

        <div>
            <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wide mb-1">Recruitment Status</label>
            <select name="state" class="w-full border-b border-gray-300 focus:border-purple-500 focus:outline-none py-1.5 text-sm bg-transparent">
                <option value="normal" {{ old('state', $job?->state) === 'normal' ? 'selected' : '' }}>Not Recruiting</option>
                <option value="open"   {{ old('state', $job?->state) === 'open'   ? 'selected' : '' }}>Recruiting</option>
            </select>
        </div>

        <div class="sm:col-span-2">
            <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wide mb-1">Job Description</label>
        ...
        <div class="sm:col-span-2">
            <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wide mb-1">Requirements</label>
