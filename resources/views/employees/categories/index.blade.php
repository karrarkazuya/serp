...
<div class="flex min-w-0 flex-col h-full bg-white">
    ...
    @if(isset($groups))
    <x-list :grouped="true" empty-text="{{ __('employees.no_categories') }}">
        <x-slot:columns>
        ...
        <tbody x-data="{ open: {{ $loop->first ? 'true' : 'false' }} }" class="divide-y divide-gray-100">
            ...
            <tr x-show="open" class="hover:bg-purple-50/30 cursor-pointer" onclick="window.location='{{ route('employees.categories.show', $category) }}'">
        ...
        @empty
        <tbody>
            <tr><td colspan="99" class="px-4 py-20 text-center text-sm text-gray-400">{{ __('employees.no_categories') }}</td></tr>
        </tbody>
        @endforelse
    </x-list>
    @else
