@extends('layouts.app')
@section('title', $resumeLineType->name)

@section('content')
<div class="flex flex-col h-full bg-gray-50">
    <div class="bg-white border-b border-gray-200 px-4 py-2 flex items-center gap-3 shrink-0">
        <div class="flex flex-col leading-tight">
            <a href="{{ route('employees.resume-line-types.index') }}" class="text-xs text-purple-600 hover:text-purple-700">{{ __('employees.resume_line_types_title') }}</a>
            <span class="text-sm font-semibold text-gray-800">{{ $resumeLineType->name }}</span>
        </div>

        <div class="flex items-center gap-2">
            @can('update', $resumeLineType)
            <a href="{{ route('employees.resume-line-types.edit', $resumeLineType) }}"
               class="px-3 py-1.5 text-sm text-gray-600 bg-white border border-gray-300 rounded hover:bg-gray-50">{{ __('common.edit') }}</a>

            @if($resumeLineType->active)
            <form method="POST" action="{{ route('employees.resume-line-types.archive', $resumeLineType) }}">
                @csrf @method('PATCH')
            ...
            @else
            <form method="POST" action="{{ route('employees.resume-line-types.unarchive', $resumeLineType) }}">
                @csrf @method('PATCH')
            ...
            <div x-data="{ confirming: false }">
                ...
                <div x-show="confirming" style="display:none" class="flex items-center gap-1.5">
                    ...
                    <form method="POST" action="{{ route('employees.resume-line-types.delete', $resumeLineType) }}">
                        @csrf @method('DELETE')
    ...
    <div class="flex-1 overflow-y-auto p-4 space-y-4">
        ...
        <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-6">
            <h2 class="text-lg font-semibold text-gray-900 mb-6">{{ $resumeLineType->name }}</h2>
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-4">
                <div>
                    <p class="text-xs font-semibold text-gray-400 uppercase tracking-wide">{{ __('common.name') }}</p>
                    <p class="text-sm text-gray-900 mt-0.5">{{ $resumeLineType->name }}</p>
                </div>
                <div>
        ...
        <div class="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
            <x-chatter
                model-type="App\Models\Employees\ResumeLineType"
                :model-id="$resumeLineType->id"
                :can-comment="auth()->user()->can('comment', $resumeLineType)"
                :comment-url="route('employees.resume-line-types.comment', $resumeLineType)"
            />
        </div>
    </div>
</div>
