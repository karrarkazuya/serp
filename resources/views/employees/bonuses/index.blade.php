...
@php
    $quickFilters = [
        ['label' => __('common.active'),   'url' => route('employees.bonuses.index', array_merge(request()->except('page'), ['filter' => '']))],
        ['label' => __('common.archived'), 'url' => route('employees.bonuses.index', array_merge(request()->except('page'), ['filter' => 'archived']))],
        ['label' => __('common.all'),      'url' => route('employees.bonuses.index', array_merge(request()->except('page'), ['filter' => 'all']))],
    ];
@endphp
...
<div class="flex min-w-0 flex-col h-full bg-white">
    <div class="flex flex-wrap items-center gap-2 sm:gap-3 px-3 sm:px-4 py-2 border-b border-gray-200 bg-white shrink-0">
        ...
        <a href="{{ route('employees.bonuses.create') }}" class="px-3 sm:px-4 py-2 bg-[#714B67] hover:bg-[#5c3d55] text-white text-sm font-semibold rounded shadow-sm shrink-0">
            {{ __('common.new') }}
        </a>
        @endcan

        <div class="flex items-center gap-1.5 min-w-0 shrink-0">
            <span class="text-xl font-semibold text-gray-700">{{ __('employees.bonuses_title') }}</span>
        </div>

        <x-search
            :model="\App\Models\Employees\EmployeeBonus::class"
            :action="route('employees.bonuses.index')"
            :quick-filters="$quickFilters"
        />

        @can('export', \App\Models\Employees\Employee::class)
        <x-export
            :fields="config('exportable')['employees.bonuses']['fields'] ?? []"
            :export-url="route('export')"
            model-key="employees.bonuses"
        />
        @endcan
    ...
    @if(isset($groups))
    <x-list :grouped="true" :empty-text="__('employees.no_bonuses')">
        <x-slot:columns>
        ...
        <tbody x-data="{ open: {{ $loop->first ? 'true' : 'false' }} }" class="divide-y divide-gray-100">
            ...
            <tr x-show="open" class="hover:bg-purple-50/30 cursor-pointer" onclick="window.location='{{ route('employees.bonuses.show', $record) }}'">
        ...
        @empty
        <tbody>
            <tr><td colspan="99" class="px-4 py-20 text-center text-sm text-gray-400">{{ __('employees.no_bonuses') }}</td></tr>
        </tbody>
        @endforelse
    </x-list>

    @else
    <x-list :paginator="$records" :selectable="true" :total-count="$records->total()" :empty-text="__('employees.no_bonuses')" :model="\App\Models\Employees\Employee::class"
            :bulk-delete-url="route('employees.bonuses.bulk-delete')">
        <x-slot:columns>
            <th class="px-4 py-2 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">{{ __('employees.doc_name') }}</th>
            <th class="px-3 py-2 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">{{ __('employees.doc_employee') }}</th>
            <th class="px-3 py-2 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">{{ __('employees.specialization_type') }}</th>
        ...
        <tr class="hover:bg-purple-50/30 cursor-pointer" onclick="window.location='{{ route('employees.bonuses.show', $record) }}'">
