...
@php
    $auth       = auth()->user();
    $defaultTab = (isset($room) && $room->isChannel()) ? 'groups' : 'direct';
    $totalGroupUnread = collect($channels ?? [])->sum(fn ($r) => $unreadCounts[$r->id] ?? 0);
    $totalDmUnread    = collect($dms ?? [])->sum(fn ($r) => $unreadCounts[$r->id] ?? 0);
@endphp

{{-- ══ Create Group modal (white overlay, uses relation-dropdown) ══ --}}
<div x-data="{ open: false }" @open-group-modal.window="open = true">
...
<div class="flex h-full overflow-hidden">
    ...
    <div class="w-64 shrink-0 flex flex-col bg-[#1e1527] overflow-hidden"
        ...
        <div class="px-4 py-4 border-b border-white/10 shrink-0">
            ...
            {{-- Tab switcher: Direct first, Groups second --}}
        ...
        <div x-show="tab === 'groups'" class="flex-1 overflow-y-auto py-2" style="display:none">
            ...
            <a href="{{ route('chat.show', $r) }}"
               class="flex items-start gap-2.5 px-3 py-2 mx-2 rounded-lg transition-colors group
                ...
                <div class="flex-1 min-w-0">
                    <div class="flex items-center gap-1.5">
                        <span class="text-sm font-semibold truncate {{ $chUnread > 0 ? 'text-white' : '' }}">{{ $r->name }}</span>
                    </div>
                    @if($r->lastMessage)
                    <div class="text-xs truncate text-white/35 mt-0.5">
    ...
    {{-- ── Right: chat + members panel ── --}}
    @if(isset($room))
    @php
        $roomDisplayName = $room->isChannel() ? $room->name : $room->displayName($auth);
        $memberCount     = $room->isChannel() ? $room->members->count() : 0;
        $existingMemberIds = $room->isChannel() ? $room->members->pluck('id')->all() : [];
    @endphp
    <div class="flex-1 min-w-0 flex overflow-hidden"
         x-data="{ membersPanel: false, addSearch: '' }">
        ...
        <div class="flex-1 min-w-0 flex flex-col bg-white overflow-hidden">
            ...
            <div class="shrink-0 px-6 py-3.5 border-b border-gray-100 flex items-center gap-3 bg-white shadow-sm">
                ...
                @if($room->isChannel())
                <button @click="membersPanel = !membersPanel"
                        :class="membersPanel ? 'bg-[#714B67]/10 text-[#714B67]' : 'text-gray-400 hover:text-gray-600 hover:bg-gray-100'"
                        class="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold transition-colors shrink-0">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div x-show="membersPanel"
             ...
             style="display:none"
             class="w-64 shrink-0 border-l border-gray-100 bg-gray-50 flex flex-col">

            {{-- Panel header --}}
            <div class="shrink-0 px-4 py-3.5 border-b border-gray-100 flex items-center justify-between bg-white">
                <span class="text-sm font-bold text-gray-800">
                    Members <span class="text-gray-400 font-normal">({{ $memberCount }})</span>
                </span>
                <button @click="membersPanel = false"
                        class="w-6 h-6 flex items-center justify-center text-gray-300 hover:text-gray-500 rounded transition-colors">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/>
                    </svg>
                </button>
            ...
            <div class="flex-1 overflow-y-auto py-2 px-2">
                @foreach($room->members->sortBy('name') as $member)
                <div class="flex items-center gap-2.5 px-2 py-2 rounded-lg hover:bg-white group">
                    <div class="w-8 h-8 rounded-full bg-[#714B67]/20 flex items-center justify-center text-[#714B67] text-xs font-bold shrink-0">
                ...
                <div class="relative">
                    ...
                    <div x-show="addSearch.length > 0"
                        ...
                        <form method="POST" action="{{ route('chat.members.add', $room) }}"
                            ...
                            <button type="submit"
                                    class="w-full text-left px-3 py-2 text-xs hover:bg-gray-50 text-gray-800 transition-colors">
