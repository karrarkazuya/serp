...
class SkillTypeController extends Controller
{
    public function read(Request $request)
    {
        ...
        if ($groupBy) {
            $fields = SearchFilters::fieldsFor(SkillType::class);
            if (isset($fields[$groupBy])) {
                $records = (clone $query)->withCount(['skills', 'levels'])->orderBy('id')->get();
                $groups  = GroupsQuery::apply($records, $fields[$groupBy]);
                return view('employees.skill-types.index', compact('groups'));
            }
        }
